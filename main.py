...
def projSceneRecBoW(feature='placeholder', classifier='placeholder'):
    ...
    if FEATURE.lower() == 'tiny_image':
        ...
        if not os.path.isfile('train_BOW.npy'):
            train_image_feats = get_bags_of_words(train_image_paths)
            np.save('train_BOW.npy', train_image_feats)
        else:
            train_image_feats = np.load('train_BOW.npy')
        ...
        if not os.path.isfile('test_BOW.npy'):
            test_image_feats = get_bags_of_words(test_image_paths)
            np.save('test_BOW.npy', test_image_feats)
        else:
            test_image_feats = np.load('test_BOW.npy')
